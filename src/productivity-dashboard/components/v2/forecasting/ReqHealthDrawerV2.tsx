 ...
 * ReqHealthDrawerV2
 *
 * Unified slide-in drawer showing requisition health, Oracle prediction, and Pre-Mortem analysis.
...
function getPriorityStyles(priority: string): { badgeClass: string; borderClass: string } {
  switch (priority) {
    ...
    case 'high':
    case 'critical':
      return { badgeClass: 'bg-bad/20 text-bad', borderClass: 'border-l-bad' };
    case 'important':
    case 'medium':
      return { badgeClass: 'bg-warn/20 text-warn', borderClass: 'border-l-warn' };
    default:
      return { badgeClass: 'bg-muted text-muted-foreground', borderClass: 'border-l-border' };
  }
}
...
interface ReqHealthDrawerV2Props {
  isOpen: boolean;
  onClose: () => void;
...
export function ReqHealthDrawerV2({
  isOpen,
  onClose,
  healthData,
  forecast,
  ...
  pipelineCandidates = [],
  reqId,
}: ReqHealthDrawerV2Props) {
  if (!isOpen || !healthData) return null;

  return (
    <GlassDrawer
      title={healthData.reqTitle}
      subtitle="Requisition Details"
      onClose={onClose}
      width="560px"
    >
      {/* Status Badges */}
      <div className="flex gap-2 items-center flex-wrap mb-4">
        <span
          className={`inline-flex items-center px-2 py-0.5 rounded-full uppercase text-[0.65rem] font-medium ${getHealthBadgeClass(healthData.healthStatus)}`}
        >
          Score: {healthData.healthScore}
        </span>
        <span
          className={`inline-flex items-center px-2 py-0.5 rounded-full uppercase text-[0.65rem] font-medium ${getVelocityBadgeClass(healthData.velocityTrend)}`}
        >
          {healthData.velocityTrend}
        </span>
        {preMortem && (
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-full uppercase text-[0.65rem] font-medium ${getRiskBandBadgeClass(preMortem.risk_band)}`}
          >
      ...
      {/* Render Oracle when simulationParams is available - it computes its own baselineForecast */}
      {/* The forecast prop is a fallback only; Oracle's baselineForecast takes precedence */}
      {simulationParams ? (
        <div className="mb-4">
          <OracleConfidenceWidgetV2
            forecast={forecast || {
              // Provide a placeholder forecast if async hasn't completed yet
              // The Oracle will compute baselineForecast from simulationParams internally
              p10Date: new Date(),
              p50Date: new Date(),
              p90Date: new Date(),
              simulatedDays: [],
              confidenceLevel: 'LOW' as const,
              debug: { iterations: 1000, seed: 'placeholder' }
            }}
            startDate={new Date()}
        ...
        <div className="glass-panel p-4 text-center mb-4 text-muted-foreground">
      ...
      {preMortem && (
        <div className="mb-4">
          ...
          <div className="p-3 bg-muted/30 rounded-lg mb-3">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                >
                  <div className="text-xs text-muted-foreground mb-0.5">Primary Failure Mode</div>
                  <div
          ...
          {preMortem.top_drivers && preMortem.top_drivers.length > 0 && (
            <div className="mb-3">
              <div className="text-xs text-muted-foreground mb-2">Risk Drivers</div>
              <div className="flex flex-col gap-2">
                {preMortem.top_drivers.slice(0, 3).map((driver, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`p-3 bg-muted/30 rounded-sm border-l-[3px] ${borderClass}`}
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right ml-3">
                          <span
                            className={`inline-flex items-center uppercase text-[0.6rem] px-1.5 py-0.5 rounded ${badgeClass}`}
                          >
                            {driver.severity}
                          </span>
          ...
          <div className="flex flex-col gap-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className={healthData.pipelineGap < 0 ? 'text-bad' : 'text-foreground'}>
                ...
                <span className="ml-1">
                  ({healthData.pipelineGap >= 0 ? '+' : ''}
                  {healthData.pipelineGap} vs benchmark)
                </span>
              </span>
            </div>
            <div className="flex justify-between">
      ...
      {/* Pipeline by Stage */}
      {healthData.candidatesByStage &&
        Object.keys(healthData.candidatesByStage).length > 0 && (
          <div className="mb-4">
            <div className="text-xs text-muted-foreground uppercase tracking-wider mb-2">
              Pipeline by Stage
            </div>
            <div className="flex gap-2 flex-wrap">
              {Object.entries(healthData.candidatesByStage).map(([stage, count]) => (
                <div key={stage} className="px-3 py-2 bg-muted/30 rounded-sm text-center">
                  <div className="font-mono font-bold text-lg text-foreground">{count}</div>
      ...
      <div>
        ...
        {healthData.actionRecommendations && healthData.actionRecommendations.length > 0 ? (
          <div className="flex flex-col gap-2">
            ...
            {preMortem?.recommended_interventions &&
              preMortem.recommended_interventions.length > 0 &&
...
export default ReqHealthDrawerV2;

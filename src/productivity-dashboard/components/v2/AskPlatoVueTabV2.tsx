  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto min-h-[calc(100vh-200px)]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6 min-h-[600px]">
        ...
        <aside className="hidden lg:block bg-[#1e293b]/70 backdrop-blur-sm rounded-xl border border-white/[0.08] p-4 h-fit max-h-[calc(100vh-250px)] overflow-y-auto">
          <div className="mb-5">
            <h2 className="text-[0.65rem] font-semibold uppercase tracking-wider text-[#64748b] mb-3 px-1">
              Suggested Questions
            </h2>
            {Object.entries(questionsByCategory).map(([category, questions]) => (
              <div key={category} className="mb-4">
                <span className="text-[0.65rem] font-semibold uppercase tracking-wide text-[#06b6d4] mb-2 block px-1">
                  {category}
                </span>
                <div className="flex flex-col gap-1">
                  {questions.map((q, idx) => (
                    <button
                      ...
                      className="text-left px-2.5 py-2 rounded-md text-sm text-[#94a3b8] hover:bg-white/[0.05] hover:text-[#f8fafc] hover:border-white/[0.08] border border-transparent transition-all"
                    >
                      {q.question}
                    </button>
                  ))}
              ...
              <div className="flex flex-col gap-1">
                {conversationHistory.slice(-5).reverse().map((item, idx) => (
                  <button
                    ...
                    className="text-left p-2 rounded-md hover:bg-white/[0.05] transition-colors"
                  >
                    <span className="block text-sm text-[#f8fafc] truncate max-w-[200px]">
                      {item.query}
                    </span>
                    <span className="block text-[0.65rem] text-[#64748b] mt-0.5">
        ...
        {/* Main Panel */}
        <main className="bg-[#1e293b]/70 backdrop-blur-sm rounded-xl border border-white/[0.08] p-6 flex flex-col min-h-[600px]">
          ...
          <div className="flex justify-end mb-4">
            {aiEnabled ? (
              ...
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-[#64748b]/10 text-[#64748b]">
          ...
          {/* Input Area */}
          <div className="flex items-end gap-3 bg-[#1e293b]/90 border border-white/[0.08] rounded-xl p-3 focus-within:border-[#06b6d4] transition-colors">
            <textarea
              ref={inputRef}
              value={query}
              ...
              rows={1}
              disabled={isLoading}
              className="flex-1 bg-transparent border-none resize-none text-[#f8fafc] text-sm placeholder:text-[#64748b] focus:outline-none min-h-[24px] max-h-[120px]"
            />
            <button
          ...
          {isLoading && (
            <div className="flex-1 flex flex-col items-center justify-center py-12">
              <div className="flex gap-2 mb-6">
                <span className="w-3 h-3 rounded-full bg-[#06b6d4] animate-pulse" style={{ animationDelay: '0ms' }} />
                <span className="w-3 h-3 rounded-full bg-[#06b6d4] animate-pulse" style={{ animationDelay: '200ms' }} />
                <span className="w-3 h-3 rounded-full bg-[#06b6d4] animate-pulse" style={{ animationDelay: '400ms' }} />
              </div>
              <div className="text-sm text-[#94a3b8] font-medium animate-pulse">
                {thinkingPhrase}
              </div>
          ...
          {response && !isLoading && (
            <div className="flex-1 flex flex-col mt-6">
                ...
                <div className="flex justify-between items-start gap-4 p-3 bg-white/[0.03] rounded-lg border border-white/[0.08] mb-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2 py-1 text-xs text-[#94a3b8] border border-white/[0.08] rounded-md hover:bg-white/[0.05] hover:text-[#06b6d4] hover:border-[#06b6d4] transition-colors"
                    >
              ...
              {/* Answer */}
              <div className="bg-[#1e293b]/90 rounded-xl p-5 mb-4 border-l-[3px] border-[#06b6d4]">
                <MarkdownRenderer
              ...
              {response.citations.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-[0.65rem] font-semibold uppercase tracking-wider text-[#64748b] mb-2">
                    Sources
                  </h4>
                      ...
                      <button
                        ...
                        className={`inline-flex items-center rounded-md overflow-hidden text-xs border hover:-translate-y-0.5 transition-all duration-200 ${
                          highlightedCitation === citation.ref
                            ? 'border-[#06b6d4] ring-2 ring-[#06b6d4]/40 scale-105 shadow-lg shadow-[#06b6d4]/20 animate-pulse'
                            : 'border-white/[0.08] hover:border-[#06b6d4]'
                        }`}
                      >
                        <span className="bg-[#06b6d4] text-[#0f172a] px-2 py-1 font-semibold text-[0.7rem]">
                          {citation.ref}
                        </span>
                        <span className="px-2.5 py-1 text-[#f8fafc] bg-[#1e293b]/90">
                          {citation.label}
                        </span>
                        <ExternalLink className="w-2.5 h-2.5 mr-2 text-[#64748b]" />
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {response.deep_links.map((link, idx) => (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-[#06b6d4] border border-[#06b6d4] rounded-md hover:bg-[#06b6d4]/10 transition-colors"
                      >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {response.suggested_questions.map((q, idx) => (
                      <button
                        ...
                        className="px-3.5 py-1.5 text-sm text-[#94a3b8] bg-[#1e293b]/90 border border-white/[0.08] rounded-full hover:border-[#06b6d4] hover:text-[#06b6d4] transition-colors"
                      >
                        {q}
                      </button>
                    ))}
              ...
              {/* Action Bar */}
              <div className="flex gap-2 pt-4 border-t border-white/[0.08] mt-auto">
                <button
                  onClick={handleCopy}
                  className={`flex items-center gap-1.5 px-3 py-2 text-xs border rounded-md transition-colors ${
                    copied
                      ? 'bg-green-500/10 border-green-500 text-green-500'
                      : 'border-white/[0.08] text-[#64748b] hover:bg-white/[0.05] hover:text-[#f8fafc]'
                  }`}
                >
                  {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                  {copied ? 'Copied!' : 'Copy'}
                </button>
                <button className="flex items-center gap-1.5 px-3 py-2 text-xs border border-white/[0.08] text-[#64748b] rounded-md hover:bg-white/[0.05] hover:text-[#f8fafc] transition-colors">
                  <ExternalLink className="w-3.5 h-3.5" />
                  View Evidence
                </button>
                <button className="flex items-center gap-1.5 px-3 py-2 text-xs bg-[#06b6d4]/10 border border-[#06b6d4] text-[#06b6d4] rounded-md hover:bg-[#06b6d4] hover:text-[#0f172a] transition-colors font-medium">
          ...
          {!response && !isLoading && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#06b6d4]/20 to-violet-500/20 flex items-center justify-center mb-5">
                <MessageSquareText className="w-6 h-6 text-[#06b6d4]" />
              </div>
              <h3 className="text-lg font-semibold text-[#f8fafc] mb-2">
                What would you like to know?
              </h3>
              <p className="text-sm text-[#94a3b8] max-w-[360px] mb-6">
                {aiEnabled
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                <button
                  ...
                  className="px-3.5 py-2 text-sm text-[#94a3b8] bg-[#1e293b]/90 border border-white/[0.08] rounded-lg hover:border-[#06b6d4] hover:text-[#06b6d4] hover:bg-[#06b6d4]/10 transition-colors"
                >
                  What's on fire?
                </button>
                <button
          ...
          {/* Data Context Footer */}
          <div className="flex gap-4 pt-3 border-t border-white/[0.08] mt-auto">
            {[
              ...
              <span key={idx} className="flex items-center gap-1.5 text-[0.65rem] text-[#64748b]">
...
function MarkdownRenderer({
  ...
  const renderLine = (line: string, key: number): React.ReactNode => {
    ...
    if (line.startsWith('### ')) {
      return (
        <h4 key={key} className="text-sm font-semibold text-[#06b6d4] mt-4 mb-2">
    ...
    if (line.startsWith('## ')) {
      return (
        <h3 key={key} className="text-base font-semibold text-[#f8fafc] mb-3 pb-2 border-b border-white/[0.08]">
    ...
    if (line.match(/^[-*]\s+/) || line.match(/^\d+\.\s+/)) {
      ...
      return (
        <li key={key} className="text-sm text-[#f8fafc] mb-1.5 ml-4">
          {renderInline(text)}
        </li>
      );
    }
    ...
    // Regular paragraph
    return (
      <p key={key} className="text-sm text-[#f8fafc] mb-2.5 leading-relaxed">
        {renderInline(line)}
      </p>
    );
  };
  ...
  const renderInline = (text: string): React.ReactNode => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="text-[#06b6d4] font-semibold">{part.slice(2, -2)}</strong>;
      }
      if (/^\[\d+\]$/.test(part)) {
        return (
          <button
            ...
            className="text-[#06b6d4] font-semibold hover:underline mx-0.5"

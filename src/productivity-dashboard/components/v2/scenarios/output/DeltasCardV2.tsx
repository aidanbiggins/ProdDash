 ...
 * V2 version using Tailwind tokens and V2 primitives.
 */
...
export function DeltasCardV2({ deltas, className = '' }: DeltasCardV2Props) {
        ...
        {displayedMetrics.map((metric) => {
          ...
          return (
            <div key={metric.key}>
              ...
              <StatValue size="md" color={isGood ? 'success' : value === 0 ? 'default' : 'danger'}>
                {isPositive ? '+' : ''}
                {value}
                {metric.suffix || ''}
              </StatValue>

  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[1055] flex items-center justify-center p-4">
        <div className="w-full max-w-2xl max-h-[90vh] flex flex-col glass-panel rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-base font-semibold text-foreground">AI Provider Settings</div>
                <p className="text-xs text-muted-foreground">Configure your AI provider and API key</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 min-h-0">
            ...
            {/* AI Enable Toggle */}
            <div className={`p-4 rounded-lg border mb-6 transition-all ${aiEnabled ? 'bg-good/10 border-good/40' : 'bg-muted/30 border-border'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${aiEnabled ? 'bg-good/20' : 'bg-muted'}`}>
                    <Zap className={`w-5 h-5 ${aiEnabled ? 'text-good' : 'text-muted-foreground/70'}`} />
                  </div>
                  <div>
                    <div className={`font-semibold ${aiEnabled ? 'text-good' : 'text-foreground'}`}>
            ...
            <div className="mb-4">
              ...
              <select
                ...
                className="w-full px-4 py-3 rounded-lg bg-muted/50 border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50"
              >
            ...
            <div className="mb-4">
              ...
              {provider === 'openai_compatible' ? (
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg bg-muted/50 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50"
                />
              ) : (
                <select
                  value={model}
                ...
                >
                  {(() => {
                    ...
                    return Array.from(categories.entries()).map(([category, categoryModels]) => (
                      <optgroup key={category} label={category} className="bg-popover text-popover-foreground">
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-3 rounded-lg bg-muted/50 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 font-mono text-sm"
                  />
                  <button
                    type="button"
                ...
                <button
                  ...
                  className="px-4 py-3 rounded-lg bg-muted/50 border border-border text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2 whitespace-nowrap"
                >
                  {isTesting ? (
              ...
              <div className="mt-2 flex items-center gap-2 text-xs">
                {testResult ? (
                  <span className={testResult.success ? 'text-good' : 'text-bad'}>
            ...
            {provider === 'openai_compatible' && (
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-lg bg-muted/50 border border-border text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 font-mono text-sm"
                />
                <p className="mt-1 text-xs text-muted-foreground/70">Must be on the approved host list</p>
              </div>
            )}
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-medium text-foreground/80">Privacy Settings</span>
              </div>

              <div
                onClick={() => setRedactPii(!redactPii)}
                className={`flex items-start gap-3 p-4 rounded-lg cursor-pointer transition-colors ${redactPii ? 'bg-good/10 border border-good/20' : 'bg-warn/10 border border-warn/20'}`}
              >
                ...
                <div>
                  <div className="font-medium text-foreground">Redact PII before sending to AI</div>
                  <div className={`text-sm mt-1 ${redactPii ? 'text-muted-foreground' : 'text-warn'}`}>
                    {redactPii
            ...
            <div className="mb-6">
              ...
              >
                ...
                />
                <span className="text-sm text-foreground">Save API key for future sessions</span>
              </div>
                ...
                <div className="mt-3 ml-8 space-y-3">
                  ...
                  >
                    <Radio
                      ...
                      name="saveScope"
                    />
                    <span className="text-sm text-muted-foreground">Save for me only</span>
                  </div>
                  {canSetOrgKey && (
                    <div
                    ...
                    >
                      <Radio
                        ...
                        name="saveScope"
                      />
                      <span className="text-sm text-muted-foreground">Share with entire organization</span>
                    </div>
                  )}
            ...
            <div>
              ...
              {showAdvanced && (
                <div className="mt-4 pl-4 border-l border-border space-y-4">
                  ...
                  {/* Max Tokens */}
                  <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-2">Max Output Tokens</label>
                    <input
                      type="number"
                      value={maxTokens}
                      ...
                      max={4096}
                      step={256}
                      className="w-full px-4 py-2 rounded-lg bg-muted/50 border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50"
                    />
                    <p className="text-xs text-muted-foreground/70 mt-1">Controls maximum response length (256-4096)</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Footer */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 px-6 py-4 border-t border-border shrink-0">
            {currentConfig && (
              <button
                type="button"
                onClick={handleClear}
                className="px-4 py-2.5 rounded-md bg-bad/10 border border-bad/30 text-bad hover:bg-bad/20 transition-colors text-sm font-medium min-h-[44px]"
              >
                Clear Config
              </button>
            )}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:ml-auto w-full sm:w-auto">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2.5 rounded-md border border-border text-muted-foreground hover:bg-muted hover:text-foreground transition-colors text-sm font-medium min-h-[44px]"
              >
                Cancel
              </button>
              <button
                ...
                onClick={handleSave}
                disabled={!isValid}
                className="px-6 py-2.5 rounded-md bg-primary text-primary-foreground font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm flex items-center justify-center gap-2 min-h-[44px]"
              >

...
export default function ExplainForExecsButton({ output }: ExplainForExecsButtonProps) {
  ...
  return (
    <>
      <button
        className="px-4 py-2 rounded-md border border-border bg-transparent text-muted-foreground hover:bg-surface-elevated hover:text-foreground transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        onClick={handleClick}
        disabled={isLoading}
      >
        {isLoading ? (

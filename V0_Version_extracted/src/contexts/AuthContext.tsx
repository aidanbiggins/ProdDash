...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        ...
        // Auto-enable dev bypass if env var is set and no existing session
        const devBypassEnvEnabled = process.env.REACT_APP_DEV_BYPASS_AUTH === 'true';
        if (devBypassEnvEnabled && !localStorage.getItem('dev-auth-bypass')) {
            console.log('[Auth] Auto-enabling dev bypass mode from env var');
            const fakeSession = {
                user: {
                    id: '00000000-0000-0000-0000-000000000099',
                    email: 'dev@platovue.local',
